...
```json
{
  "error": {
    ...
    "message": "Cannot send free-form message. Window closed. Use a template instead.",
    "details": {
      "diagnostic": "63016"
    }
  }
...
Se cliente respondeu há 30s mas o webhook ainda não chegou na Arara, `window-status` pode dizer `CLOSED`. Solução: espere alguns segundos OU faça retry quando receber `UNPROCESSABLE_ENTITY` (422).
