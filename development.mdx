...
- **Gestão de Mensagens**: Métodos para `POST /v1/messages` e `GET /v1/messages/{internalId}`
    ...
    export class Messages {
      ...
      async send(params: SendMessageParams): Promise<Message> {
        ...
        if (!params.receiver.startsWith('whatsapp:+')) {
          throw new Error('Receiver must be in format whatsapp:+5511999999999');
        }
    ...
    class Messages:
        ...
        def send(self, params: SendMessageParams) -> Message:
            ...
            if not params.receiver.startswith('whatsapp:+'):
                raise ValueError(
                    'Receiver must be in format whatsapp:+5511999999999'
                )
