...
| `media_url` | string | Não | URL pública da mídia (Imagem, PDF) para templates com header de mídia |
...
Se o seu template foi criado com um **Header de Mídia** (Imagem, PDF ou Vídeo), você deve enviar a URL pública do arquivo no campo `media_url`.
