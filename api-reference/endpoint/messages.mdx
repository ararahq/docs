---
title: 'Mensagens (V1)'
description: 'Envie mensagens e consulte o status via Arara — V1 Legacy'
---

<Warning>
  **Esta e a documentacao da V1 (legacy).** A V2 tem uma API mais limpa, multi-canal (WhatsApp + Email) e respostas sem dados internos. [Ver documentacao V2](/api-reference/endpoint/v2/messages)
</Warning>
...
|-------|------|-------------|-----------|
| `receiver` | string | Sim | Número do destinatário no formato `whatsapp:+5511999999999` |
| `templateName` | string | Condicional* | Nome do template que foi criado no dashboard (obrigatório para mensagens de template) |
| `variables` | array | Não | Lista de variáveis para preencher o template. Se o template tiver Header (Imagem ou PDF), a URL deve ser enviada na **primeira posição** (`variables[0]`). |
...
- DDD: `83`
- Número: `991768778`
- **Formato completo**: `whatsapp:+5511999999999`
...
### Enviar Mensagem com Mídia (Header de Mídia)

Se o seu template foi criado com um **Header de Mídia** (Imagem, PDF ou Vídeo), você deve enviar a URL pública do arquivo na **primeira posição** do array `variables`.
...
curl -X POST https://api.ararahq.com/api/v1/messages \
  ...
  -d '{
    "receiver": "whatsapp:+5511999999999",
    "templateName": "mensagem_com_anexo_pdf",
    "variables": ["https://ararahq.com/l/FtFmja", "João"]
  }'
```

<Tip>
  **Dica**: Você pode usar o próprio dashboard da Arara para fazer upload da mídia e obter um Smart Link encurtado, garantindo tracking de cliques e melhor performance.
</Tip>
...
```json
{
  ...
  "message": "Invalid receiver format. Expected format: whatsapp:+5511999999999"
}
```
...
String jsonBody = mapper.writeValueAsString(Map.of(
    "receiver", "whatsapp:+5511999999999",
    "templateName", "template_teste_inicio_conversa",
    "variables", List.of("https://ararahq.com/l/FtFmja", "João"),
    "scheduled_at", "2024-12-25T10:00:00Z"
));
