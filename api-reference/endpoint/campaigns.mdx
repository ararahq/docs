...
<Warning>
  **Esta e a documentacao da V1 (legacy).** [Ver documentacao V2](/api-reference/endpoint/v2/campaigns)
</Warning>
...
As variáveis seguem a ordem indexada do template (ex: `{{1}}`, `{{2}}`). Ao enviar o array `variables`, a Arara mapeia o primeiro item para `{{1}}`, o segundo para `{{2}}` e assim por diante.

**Exemplo:**
Se seu template é: `Olá {{1}}, seu pedido {{2}} foi enviado!`
O array deve ser: `["João", "12345"]`
...
```json
{
  ...
  "templateName": "template_campanha_v1",
  "contacts": [
    {
...
Para campanhas de larga escala, a Arara oferece notificações automáticas via webhook para que seu sistema saiba exatamente o que está acontecendo sem precisar consultar a API repetidamente.

### Evento: `campaign.completed`

Este evento é disparado assim que a Arara termina de processar todos os contatos da sua campanha e os coloca na fila de disparo.
...
```json
{
  ...
  "timestamp": "2024-02-22T17:40:26Z",
  "organizationId": "ara_org_...",
  "data": {
...
<Tip>
  Além deste evento de conclusão de campanha, você continuará recebendo webhooks individuais (`message.status_updated`) para cada mensagem entregue ou lida dentro da campanha.
</Tip>
