...
| `charge.expiresAt` | Opcional, epoch em segundos, pelo menos 5 minutos à frente. Quando passa sem pagamento, a cobrança vira `EXPIRED` sozinha. |
...
Existe ainda o status `EXPIRED`, que você não envia: a Arara marca sozinha quando o `expiresAt` passa. No WhatsApp o cartão aparece como cancelado, porque ele não tem um estado de vencido.
...
```bash
curl "https://api.ararahq.com/v1/charges?page=0&size=20&status=PENDING" \
  -H "Authorization: Bearer ara_live_xxx"
```
...
```json
{
  ...
  "status": "PAID",
  "source": "HOOK",
  "contact": { "id": "a1b2c3d4-...", "name": "Maria", "phone": "5588999990000" },
...
| `PUT /v1/charges/{id}/status` | Dá baixa direto: `{ "status": "PAID" \| "CANCELED" \| "FAILED" }`. Funciona com a conversa fechada; o cartão no WhatsApp só é atualizado quando a janela de 24h deixa (`cardUpdated` na resposta). |
