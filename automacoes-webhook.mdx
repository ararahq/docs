...
```
POST https://api.ararahq.com/hooks/a/{token}
```
...
| `error.code` | Quando |
|---|---|
| `HOOK_NOT_FOUND` | O token não existe, ou o link foi regenerado. |
| `HOOK_SIGNATURE_INVALID` | A assinatura é exigida e o header está ausente ou não confere com o corpo. |
| `HOOK_PAYLOAD_INVALID` | O corpo não é nenhum dos dois formatos, ou falta campo obrigatório. |
| `INVALID_CHARGE` | A cobrança não passou nas conferências (Pix inválido, valor, `referenceId`...). |
| `AUTOMATION_INACTIVE` | A automação está desligada. |
| `CHARGE_NOT_FOUND` | Não existe cobrança com esse `referenceId` enviada por esta automação. |
| `CHARGE_ALREADY_SETTLED` | A cobrança já tem um status final diferente do enviado. |
